...
export class AutomationPesquisa{
    ...
    static visualizarLayout(layout){
        cy.get('#fragment-0-ehmo > .component-heading > div').should('contain', 'Guia Médico');
    }
    ...
    static pesquisar_medicosRiodeJaneiro(medicos){
        cy.get('.AutoSuggest__value-container').type('Médicos no Rio de Janeiro');
    }
    ...
    static visualizar_EspecialidadeCidade(EspecialidadeCidade){
        cy.wait(15000);
	    cy.get('#gm-v3-root > :nth-child(1) > :nth-child(2)').should('contain', 'Rio de Janeiro/RJ');
	    cy.get(':nth-child(1) > .ProviderCard > .Provider--infos > .ProviderSpecialties > .ProviderSpecialties--item').should('be.visible', 'Specialties').screenshot();
    }

    static visualizar_naoconterSP(naoconter){
        cy.wait(15000);
        cy.get('#gm-v3-root > :nth-child(1) > :nth-child(2)').should('not.contain.text', 'São Paulo/SP').screenshot();
    }
